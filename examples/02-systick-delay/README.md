...
## 3) The `LongStopWatch` class

For this scenario the `LongStopWatch` class is the solution. This is an 
example:
...
Make two instances of `LongStopWatch` with different duration and modify 
...
toggle. For this use the `LongStopWatch::Continue()` method.

Important to note here is that the design of `LongStopWatch::Continue()` 
does not resets the time base, as opposed to `LongStopWatch::Restart()`, 
which means that no accumulated time shift error will happen when you 
continually *continues* the instance.

If one uses `LongStopWatch::Restart()` a new time reference is sampled, 
